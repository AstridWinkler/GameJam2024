...
namespace UnityEditor
{
    ...
    internal class RuleTileEditor : Editor
    {
        ...
        private const string s_Arrow10 = "";
        ...
        public static Texture2D[] arrows
        {
            get
            {
                if (s_Arrows == null)
                {
                    ...
                    s_Arrows[10] = Base64ToTexture(s_Arrow10);
        ...
        internal void RuleNeighborUpdate(Rect rect, RuleTile.TilingRule tilingRule, int index)
        {
            if (Event.current.type == EventType.MouseDown && ContainsMousePosition(rect))
            {
                ...
                if (Event.current.modifiers == EventModifiers.Shift)
                    newIndex = RuleTile.TilingRule.Neighbor.Empty;
                
                if (Event.current.modifiers == EventModifiers.Alt)
                    newIndex = RuleTile.TilingRule.Neighbor.DontCare;

                if (Event.current.modifiers == EventModifiers.Control)
                    newIndex = RuleTile.TilingRule.Neighbor.NotThis;
