    ...
    public class TilemapManager
    {
        ...
        void Initialize()
        {
            if (!initialized)
            {
                ...
                display = new LightTile[Lighting2D.ProjectSettings.chunks.maximumTilesTilemap];
                maps = new List<LightTile>[Lighting2D.ProjectSettings.chunks.maximumTilesTilemap/10, Lighting2D.ProjectSettings.chunks.maximumTilesTilemap/10];
            }
        }
        ...
        private void GenerateChunks(Rect worldRect)
        {
            ...
            for(int x = tp0.x; x <= tp1.x; x++)
            {
                for(int y = tp0.y; y <= tp1.y; y++)
                {
                    ...
                    {
                        if (distplayCount < Lighting2D.ProjectSettings.chunks.maximumTilesTilemap)
                        {
